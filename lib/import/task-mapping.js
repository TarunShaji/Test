...
const TASK_STATUS_MAP = {
    ...
    'pending': 'To Be Started', 'to do': 'To Be Started', 'todo': 'To Be Started',
}
...
export function mapTaskStatus(s) {
    ...
    const norm = normHeader(s)
    return TASK_STATUS_MAP[norm] || 'To Be Started'
}
...
export function mapTaskPriority(s) {
    if (!s) return 'P2'
    const norm = normHeader(s)
    return TASK_PRIORITY_MAP[norm] || 'P2'
}
...
const INTERNAL_APPROVAL_MAP = {
    'pending': 'Pending',
    'approved': 'Approved',
}

export function mapInternalApproval(s) {
    if (!s) return 'Pending'
    const norm = normHeader(s)
    return INTERNAL_APPROVAL_MAP[norm] || 'Pending'
}
...
export const TASK_HEADER_KEYWORDS = {
    ...
    _ignored_approval: ['send link', 'client approval', 'client feedback'],
}
...
export function rowToTask(row, headers, clientId) {
    ...
    // ── Enum: internal_approval (case-insensitive) ───────────────────────────
    const internalCol = h('internal_approval')
    if (internalCol) {
        const val = safeStr(row[internalCol])
        if (val) {
            task.internal_approval = mapInternalApproval(val)
            console.debug(`[task-mapping] ✅ internal_approval ← "${internalCol}" = "${val}" → "${task.internal_approval}"`)
        }
    }
