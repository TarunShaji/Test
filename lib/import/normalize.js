...
/** Normalize a column header: lowercase, collapse all separators to a single space */
 ...
 *  Prioritizes exact matches, then fuzzy substring matches. */
export const findHeader = (headers, keywords) => {
    // 1. Exact match pass (normalized)
    const exact = headers.find(h => keywords.includes(normHeader(h)))
    if (exact) return exact

    // 2. Fuzzy substring match fallback
    // Matches if header contains keyword OR keyword contains header (handles typos/truncation)
    return headers.find(h => {
        const nh = normHeader(h)
        if (!nh) return false
        return keywords.some(k => nh.includes(k) || k.includes(nh))
    }) || null
}
